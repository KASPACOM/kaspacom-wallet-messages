...
export class KaspaComWebWalletMessagesService {
  ...
  private eventHandlers: KaspaComWebWalletMessagesEventHandlers[] = [];
  ...
  private waitForWalletToConnectResolve:
    | ((walletInfo: WalletInfoPayloadInterface) => void)
    | undefined = undefined;
  private waitForWalletToConnectReject: (() => void) | undefined = undefined;

  protected isFirstEventEmitted: boolean = false;
   ...
   */
  constructor(protected walletUrl: string) {
   ...
   * ```
   */
  public async startConnectionAndWaitForWalletSelection(iframe: HTMLIFrameElement, eventHandlers: KaspaComWebWalletMessagesEventHandlers): Promise<WalletInfoPayloadInterface> {
    this.iframe = iframe;
  ...
  private async onMessage(event: any): Promise<void> {
    if (
      new URL(event.origin).origin === new URL(this.walletUrl).origin &&
      event.data &&
      event.data?.type
    ) {
  ...
  addEventHandlers(eventHandlers: KaspaComWebWalletMessagesEventHandlers) {
    this.eventHandlers.push(eventHandlers);
  }

  removeEventHandlers(eventHandlers: KaspaComWebWalletMessagesEventHandlers) {
